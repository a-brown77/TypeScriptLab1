...
// Override greet: Override the greet method to add the extra exclamation points to the end. For example, greet might return "Hello, Grant!!!". Note, the original greetmethod already has one exclamation point, so LoudGreeter starts with two exclamation points even before calling addVolume.
...
class LoudGreeter extends Greeter {
  ...
  greet(name: string): string {
    return `${this.greeting}, ${name}!${this._extra}`;
  }
}
